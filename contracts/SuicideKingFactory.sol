...
import "./IFactory.sol";
import "./SuicideKing.sol";
import "./Strings.sol";

// WIP
contract SuicideKingFactory is IFactory, Ownable, ReentrancyGuard {
  ...
  /**
   * Three different options for minting SuicideKings (basic, premium, and gold).
   */
  enum Option {
    Basic,
    Premium,
  ...
  function _mint(
    ...
    require(_canMint(msg.sender, _option, _amount), "SuicideKing#_mint: CANNOT_MINT_MORE");
    uint256 optionId = uint256(_option);
    SuicideKing nftContract = SuicideKing(nftAddress);
